...
//BEGIN ClientGroupState

ClientGroupState::ClientGroupState(void*, int, IPAddress) {}
...
//BEGIN RouterGroupState

RouterGroupState::RouterGroupState(void* _table, int _interface, IPAddress _group):
		table((RouterMCTable*) _table), interface(_interface), group(_group) {
	init_timer();
}
